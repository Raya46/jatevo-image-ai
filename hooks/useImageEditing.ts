...
interface UseImageEditingReturn {
  ...
  removeBackground: (imageUri: string, onProgress?: (progress: number) => void) => Promise<GalleryImage | null>;
  enhanceImage: (imageUri: string, onProgress?: (progress: number) => void) => Promise<GalleryImage | null>;
  adjustColors: (imageUri: string, adjustments: string, onProgress?: (progress: number) => void) => Promise<GalleryImage | null>;

  cropImage: (imageUri: string, cropParams: any, onProgress?: (progress: number) => void) => Promise<GalleryImage | null>;
  applyFilter: (imageUri: string, filterType: string, onProgress?: (progress: number) => void) => Promise<GalleryImage | null>;


  customEdit: (imageUri: string, instructions: string, onProgress?: (progress: number) => void) => Promise<GalleryImage | null>;
...
export const useImageEditing = (): UseImageEditingReturn => {
  ...
  const removeBackground = useCallback(async (imageUri: string, onProgress?: (progress: number) => void): Promise<GalleryImage | null> => {
    ...
    try {
      ...
      return await safeEdit(imageUri, prompt, 'Background Removal', onProgress);
    } catch (err) {
  ...
  const enhanceImage = useCallback(async (imageUri: string, onProgress?: (progress: number) => void): Promise<GalleryImage | null> => {
    setIsProcessing(true);
    try {
      ...
      return await safeEdit(imageUri, prompt, 'Image Enhancement', onProgress);
    } catch (err) {
